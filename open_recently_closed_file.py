...
# TODO use api function to get current project name rather than a hash of
# the folder names
...
class OpenRecentlyClosedFileEvent(sublime_plugin.EventListener):
    ...
    def on_close(self, view):
        ...
        self.add_to_history(
            view, project_name + '_closed', project_name + '_opened')
    ...
    def on_load(self, view):
        ...
        project_name = get_current_project_hash()
        self.add_to_history(
            view, project_name + '_opened', project_name + '_closed')

    def add_to_history(self, view, add_to_setting, remove_from_setting):
        filename = os.path.normpath(
            view.file_name()) if view.file_name() else None
        if filename != None:
            ...
            # add this file to the top of the "add_to_list" (but only if the
            # file actually exists)
            if os.path.exists(filename):
                add_to_list.insert(0, filename)

            # write the history back (making sure to limit the length of the
            # histories)
            set_history(add_to_setting, add_to_list[0:HISTORY_MAX_ENTRIES])
            set_history(remove_from_setting, remove_from_list[
                        0:HISTORY_MAX_ENTRIES])
...
class OpenRecentlyClosedFileCommand(sublime_plugin.WindowCommand):
    ...
    def run(self, show_quick_panel=True, current_project_only=True):
        ...
        if show_quick_panel:
            self.window.show_quick_panel(
                self.display_list, self.open_file, True)
        else:
            self.open_file(0)
    ...
    def reload_history(self, current_project_only):
        ...
        if current_project_only:
            project_name = get_current_project_hash()
            self.file_list = get_history(
                project_name + '_closed') + get_history(project_name + '_opened')
        else:
        ...
        for filePath in self.file_list:
            self.display_list.append([os.path.basename(
